...
export function GoalCard({
  ...
  async function handleDeposit() {
    ...
    if ("error" in res) toast(res.error ?? "Lỗi", "error");
      ...
      toast(`Đã nạp ${formatCurrency(amount)}! 🎉`, "success");
  ...
  async function handleDelete() {
    if (!confirm(`Xóa mục tiêu "${g.name}"? Hành động này không thể hoàn tác.`))
      return;
    setDeleting(true);
  ...
  async function handleEdit() {
    ...
    fd.set("target_amount", editAmt);
    fd.set("deadline", editDL);
    fd.set("priority", editPri);
  ...
  return (
    <>
      <div
        className={`bg-white rounded-xl shadow-sm overflow-hidden transition-shadow group ${
          completed
            ? "border border-green-200 opacity-85"
            : "border border-gray-100 hover:shadow-md"
        }`}
      >
        {/* Cover */}
        <div
          className="h-28 flex items-end relative"
          style={
            g.cover_url
              ? {
        ...
        >
          ...
          <span
            className={`absolute top-2 right-2 text-xs font-semibold px-2 py-0.5 rounded-full transition-opacity group-hover:opacity-0 ${priority.cls}`}
          >
            {priority.label}
          </span>
          {isOverdue && (
            <span className="absolute top-2 left-2 text-xs font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700">
              Quá hạn
            </span>
          )}
          {completed && (
          ...
          {!completed && (
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
              <button
                ...
                className="p-1.5 bg-white/80 rounded-lg text-gray-700 hover:bg-white transition-colors text-xs"
              >
                ✏️
              </button>
              <button
          ...
          <p className="relative z-10 px-4 pb-3 text-white font-bold text-base drop-shadow truncate w-full">
            {g.name}
          </p>
        </div>
        ...
        <div className="p-4 space-y-3">
          <p className="text-xs text-gray-400">
            ...
            {daysLeft > 0 && !completed && (
              <span
                className={`ml-1 font-medium ${daysLeft <= 7 ? "text-red-500" : "text-gray-500"}`}
              >
                · còn {daysLeft} ngày
              </span>
            )}
          ...
          <div className="flex justify-between text-xs text-gray-500">
            ...
            <span>
              Mục tiêu: <strong>{formatCurrency(g.target_amount)}</strong>
            </span>
          </div>
          {!completed && remaining > 0 && dailyNeeded > 0 && (
            <p className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2">
              💡 Cần{" "}
              <strong className="text-indigo-600">
                {formatCurrency(Math.ceil(dailyNeeded))}/ngày
              </strong>{" "}
              để đúng hạn
            </p>
          )}
          {!completed ? (
            ...
            <div className="w-full py-2 bg-green-50 text-green-700 rounded-lg text-sm font-semibold text-center">
              ✅ Đã hoàn thành!
            </div>
          )}
        </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {remaining > 0 && (
              <button
                onClick={() => setDepositAmt(String(Math.ceil(remaining)))}
