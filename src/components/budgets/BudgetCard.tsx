...
export function BudgetCard({
  ...
  const status =
    b.percentage >= 100
      ? "Vượt hạn mức"
      : b.percentage >= b.alert_at_percent
        ? "Sắp hết"
        : "Bình thường";
  const statusColor =
    b.percentage >= 100
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    if ("error" in res) toast(res.error ?? "Lỗi xóa ngân sách", "error");
      ...
      toast("Đã xóa ngân sách", "success");
      onDeleted?.();
    }
  }
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow group relative">
      ...
      <div className="mt-3 flex justify-between text-xs text-gray-500">
        <span>
          Đã dùng:{" "}
          <strong className="text-gray-800">{formatCurrency(b.spent)}</strong>
        </span>
        <span>
          Còn lại:{" "}
          <strong
            className={b.remaining < 0 ? "text-red-600" : "text-green-600"}
          >
      ...
      <p className="mt-1 text-xs text-gray-400 text-right">
        Tổng: {formatCurrency(b.amount_limit)}
      </p>
