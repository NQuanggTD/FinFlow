...
import { BudgetAlertToast } from "@/components/budgets/BudgetAlertToast";
...
export function TransactionForm({ categories, accounts, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (receiptFile) {
      try {
        ...
        if (data.url) fd.set("receipt_url", data.url);
          ...
          toast("Không thể upload hóa đơn, tiếp tục lưu giao dịch", "warning");
      } catch {
    ...
    if (result && "error" in result) {
      ...
      if ("budgetAlert" in result && result.budgetAlert) {
        setBudgetAlert(result.budgetAlert);
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-indigo-300 hover:bg-indigo-50/30 transition-colors">
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
