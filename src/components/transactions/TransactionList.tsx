...
import { useTransactions, type TransactionWithRelations } from "@/hooks/useTransactions";
...
export function TransactionList({ initialData, month, year }: Props) {
  ...
  const { data: fetched, isLoading } = useTransactions(currentMonth, currentYear);
  ...
  const grouped = paginated.reduce<Record<string, TransactionWithRelations[]>>((acc, t) => {
  ...
  return (
    <div className="space-y-4">
      <TransactionFilters
        ...
        onMonthChange={setCurrentMonth} onYearChange={setCurrentYear}
        ...
        <>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center justify-between text-sm">
            ...
            <div className="flex gap-4">
              <span className="text-green-600 font-medium">Thu: +{formatCurrency(totalIncome)}</span>
              <span className="text-red-500  font-medium">Chi: −{formatCurrency(totalExpense)}</span>
              <span className={`font-semibold ${totalIncome - totalExpense >= 0 ? "text-indigo-600" : "text-orange-500"}`}>
