...
export function AIChat() {
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: data.reply ?? data.error ?? "Xin lỗi, tôi không thể trả lời ngay lúc này.",
          ts: Date.now(),
        },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "❌ Lỗi kết nối. Vui lòng thử lại sau.", ts: Date.now() },
      ]);
    }
    setLoading(false);
  }, [input, loading]);
  ...
  return (
    <Card className="flex flex-col" style={{ height: "520px" }}>
      <CardHeader title="💬 Chat với AI" subtitle="Hỏi về tài chính cá nhân của bạn" />
