...
const TYPE_CFG: Record<
  AIInsight["type"],
  { icon: string; color: "indigo" | "amber" | "green" }
> = {
      ...
      {!fetched && !loading && !errMsg && (
        <div className="flex flex-col items-center py-10 text-center">
          ...
          <p className="font-semibold text-gray-800 mb-1">
            AI sẵn sàng phân tích
          </p>
          <p className="text-sm text-gray-400 max-w-xs">
            Nhấn "Phân tích" để nhận nhận xét cá nhân hoá dựa trên chi tiêu 30
            ngày qua.
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="p-4 border border-gray-100 rounded-xl space-y-2 animate-pulse"
            >
