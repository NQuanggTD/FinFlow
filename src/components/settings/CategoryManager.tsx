...
export function CategoryManager() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    const { data } = await (sb.from("categories") as any)
      .select("*")
      .order("name");
  ...
  async function handleDelete(id: string, catName: string, isDefault: boolean) {
    if (isDefault) {
      toast("Không thể xóa danh mục mặc định", "warning");
      return;
    }
  ...
  return (
    <div suppressHydrationWarning>
      <Card>
        <CardHeader
          title="🏷️ Danh mục giao dịch"
          subtitle="Quản lý danh mục tuỳ chỉnh của bạn"
        />
