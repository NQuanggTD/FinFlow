...
export function DashboardShell({
  ...
  const [showAdminConfirmed, setShowAdminConfirmed] = useState(
    () => search?.get("admin_confirmed") === "1",
  );
  ...
  return (
    <div className="space-y-6">
      {showAdminConfirmed && (
        <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg text-sm text-emerald-700 flex items-center justify-between">
          <div>✅ Tài khoản đã được xác thực tự động — bạn đã đăng nhập.</div>
          <button
            className="text-emerald-700 font-semibold"
            onClick={() => setShowAdminConfirmed(false)}
          >
            Đóng
          </button>
        </div>
      ...
      <div className="w-full rounded-lg border border-gray-100 bg-white p-4">
        ...
        <p className="mt-2 text-sm text-gray-500">
          Biểu đồ so sánh tạm thời không khả dụng.
        </p>
      </div>
