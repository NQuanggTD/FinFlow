...
export function OnboardingBanner({ hasAccounts, hasTransactions }: Props) {
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-5 mb-2">
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h3 className="font-semibold text-indigo-900">🎉 Chào mừng đến với FinFlow!</h3>
          <p className="text-sm text-indigo-600 mt-0.5">
            Hoàn thành {done}/{total} bước để bắt đầu
          </p>
        </div>
        <span className="text-xl font-bold text-indigo-400 ml-4">{done}/{total}</span>
      </div>
      ...
      <div className="space-y-2">
        {steps.map((step, i) => (
          <div key={i}
            ...
            <span className={`text-sm flex-1 ${step.done ? "line-through text-gray-400" : "text-gray-700"}`}>
